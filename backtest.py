...
class Strategy:
    ...
    def _broken_wing_butterfly_with_puts(self, today, last):
        # a.k.a. skip strike butterfly with puts
        pxA = round(last * (1 - 2 * self._put_offset), 2)
        pxB = last
        pxC = round(last * (1 + 1 * self._put_offset), 2)
        pxD = round(last * (1 + 4 * self._put_offset), 2)
        ltd = str((today + pd.DateOffset(days=self._days_to_maturity)).date())
        qty = int(self._acc.net_asset_value(today)[-1]*self._target_lv/last/100)

        self._acc.close_all_option_positions(today)
        self._acc.trade_option(today, Option('put', self._stock_ticker, pxA, ltd), +1 * qty)
        self._acc.trade_option(today, Option('put', self._stock_ticker, pxC, ltd), -2 * qty)
        self._acc.trade_option(today, Option('put', self._stock_ticker, pxD, ltd), +1 * qty)
    ...
    def run(self, **kwargs):
        ...
        for i,today in enumerate(timeline):
            ...
            if i%self._rebal_freq==0:
                ma50 = self._stock_prices[self._stock_ticker].loc[:today].iloc[-50:].mean()
                ma200 = self._stock_prices[self._stock_ticker].loc[:today].iloc[-200:].mean()
                if ma50>=ma200:
                    # Bull market
                    ## Strategy
                    self._acc.close_all_option_positions(today)
                    ## Benchmark
                    self._acc_bm.close_all_stock_position(today)
                    #
                else:
                    # Bear market
                    ## Strategy
                    self._broken_wing_butterfly_with_puts(today, last)
                    ## Benchmark
                    self._acc_bm.trade_stock_target_percentage(today, Stock(self._stock_ticker), 1.0)
                    #
...
if __name__ == "__main__":
    Strategy(
        ...
        days_to_maturity=45,    # option days to maturity, in normal day
